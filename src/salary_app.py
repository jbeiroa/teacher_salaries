...
# --- Data Loading ---
loader = DataLoader()
scraper = loader.scraper  # Re-expose scraper for utility methods in callbacks
all_data = loader.get_all_data()

# Pre-load and align data to start from Dec 2016 (INDEC IPC start)
START_LIMIT = "2016-12-01"

df_net_salary = all_data["net_salaries"].loc[START_LIMIT:]
df_gross_salary = all_data["gross_salaries"].loc[START_LIMIT:]
df_basic_salary = all_data["basic_salaries"].loc[START_LIMIT:]
df_ipc = all_data["inflation_ipc"]
df_cba_cbt = all_data["poverty_lines"].loc[START_LIMIT:]

df_clusters = all_data.get("clusters")
df_anomalies = all_data.get("anomalies")
    ...
    with open(final_path, "r") as f:
        ...
        if cluster_start_idx != -1:
            ...
            for s in sections[cluster_start_idx:]:
                if "## **Cluster" in s:
                    REPORT_SECTIONS[lang]["clusters"].append(
                        process_citations(s.strip())
                    )
                else:
...
TRANSLATIONS = {
    "es": {
        ...
        "instr_body": [
            html.P(
                "Este tablero permite analizar la evolución del poder adquisitivo de los docentes en Argentina."
            ),
            html.B("Filtros Principales:"),
            html.Ul(
                [
                    html.Li(
                        "Provincia y Tipo de Salario: Seleccione la jurisdicción y si desea ver el Neto (bolsillo), Bruto o Básico."
                    ),
                    html.Li(
                        "Mostrar Real: Activa el ajuste por inflación. Si está desactivado, verá valores nominales."
                    ),
                    html.Li(
                        "Mostrar Línea de Ref: Muestra canastas básicas (CBA/CBT) o líneas de pobreza/indigencia para comparar el salario frente al costo de vida."
                    ),
                    html.Li(
                        "Mostrar como Índice: Transforma los gráficos en porcentaje de crecimiento relativo a la 'Fecha Base' seleccionada. Útil para comparar quién ganó o perdió más desde un momento específico."
                    ),
                ]
            ),
            ...
            html.Ul(
                [
                    html.Li(
                        "Icono ⚠️: Indica una caída anómala en el salario real (pérdida brusca de poder adquisitivo)."
                    ),
                    html.Li(
                        "Icono ✨: Indica un aumento anómalo (generalmente bonos extraordinarios o retroactivos)."
                    ),
                    html.Li(
                        "Pestaña Analítica Avanzada: Muestra grupos de provincias (Clusters) con comportamientos similares identificados mediante Machine Learning."
                    ),
                ]
            ),
        ],
    ...
    "en": {
        ...
        "instr_body": [
            html.P(
                "This dashboard analyzes the evolution of teacher purchasing power in Argentina."
            ),
            html.B("Main Filters:"),
            html.Ul(
                [
                    html.Li(
                        "Province & Salary Type: Select the jurisdiction and whether to see Net (take-home), Gross, or Basic salary."
                    ),
                    html.Li(
                        "Show Real: Enables inflation adjustment. If off, you see nominal values."
                    ),
                    html.Li(
                        "Show Ref. Line: Displays basic baskets (CBA/CBT) or poverty/indigency lines to compare salary against cost of living."
                    ),
                    html.Li(
                        "Show as Index: Transforms charts into percentage growth relative to the selected 'Base Date'. Useful for comparing relative gains or losses from a specific point in time."
                    ),
                ]
            ),
            ...
            html.Ul(
                [
                    html.Li(
                        "⚠️ Icon: Indicates an anomalous drop in real salary (sharp loss of purchasing power)."
                    ),
                    html.Li(
                        "✨ Icon: Indicates an anomalous increase (usually extraordinary bonuses or back-pay)."
                    ),
                    html.Li(
                        "Advanced Analytics Tab: Displays groups of provinces (Clusters) with similar behaviors identified through Machine Learning."
                    ),
                ]
            ),
        ],
...
def create_kpi_card(
    ...
    if (
        nom_variation is not None
        and not pd.isna(nom_variation)
        and ipc_variation is not None
    ):
    ...
    return dbc.Card(
        dbc.CardBody(
            [
                html.H6(
                    title,
                    className="card-subtitle text-muted mb-1 text-center",
                    style={"fontSize": "0.85rem"},
                ),
                html.H4(
                    nominal_value,
                    className="card-title text-dark mb-1 text-center",
                ...
                html.Div(
                    [
                        ...
                        html.Small(
                            var_real_text,
                            className=f"{color_real} fw-bold d-block text-center",
                            style={"fontSize": "0.85rem", "minHeight": "1.2rem"},
                        ),
                    ]
                ),
            ],
    ...
    [
        ...
        dbc.Row(
            [
                ...
                dbc.Col(
                    [html.H1(id="app-title", className="text-center mt-4")], width=10
                ),
                dbc.Col(
                    [
                        html.Div(
                ...
                dbc.Col(
                    [
                        dbc.Card(
                            [
                                dbc.CardHeader(
                                    id="sidebar-header", className="fw-bold"
                                ),
                                dbc.CardBody(
                                    [
                                        html.Label(
                                            id="label-province", className="mt-2"
                                        ),
                                        dcc.Dropdown(
                                            id="province-dropdown",
                                            options=[
                                                {"label": col, "value": col}
                                                for col in df_net_salary.columns
                                            ],
                                            value="Chaco",
                                            clearable=False,
                                            className="mb-3",
                                        ...
                                        dcc.Dropdown(
                                            ...
                                            options=[
                                                ...
                                                for d in df_ipc.index
                                            ],
                                            value=df_net_salary.index[-1].strftime(
                                                "%Y-%m-%d"
                                            ),
                                            clearable=False,
                                            className="mb-3",
                                        ...
                                        dcc.DatePickerRange(
                                            ...
                                            start_date=df_net_salary.index[
                                                0
                                            ],  # Dec 2016
                        ...
                        dbc.Tabs(
                            [
                                dbc.Tab(
                                    ...
                                    children=[
                                        ...
                                        dbc.Row(
                                            [
                                                dbc.Col(
                                                    id="kpi-latest", width=12, md=3
                                                ),
                                                dbc.Col(
                                                    id="kpi-quarterly", width=12, md=3
                                                ),
                                                dbc.Col(
                                                    id="kpi-annual", width=12, md=3
                                                ),
                                                dbc.Col(
                                                    id="kpi-interannual", width=12, md=3
                                                ),
                                            ],
                                            className="mt-3 mb-4",
                                        ),
                                                    ...
                                                    [
                                                        dbc.Card(
                                                            [
                                                                ...
                                                                dbc.CardBody(
                                                                    dcc.Graph(
                                                                        id="provincial-comparison-chart",
                                                                        config={
                                                                            "displayModeBar": False
                                                                        },
                                                                        style={
                                ...
                                dbc.Tab(
                                    ...
                                    children=[
                                        dcc.Store(
                                            id="analytics-carousel-index", data=0
                                        ),
                                        dbc.Row(
                                            [
                                                dbc.Col(
...
def update_dashboard(
    ...
    metrics = get_variation_metrics(
        df_nom[selected_province], df_real[selected_province], ipc_series
    )

    # KPI components
    latest_nom_fmt = f"${format_localized(metrics['latest_nom'], lang=lang)}"
    kpi_latest = create_kpi_card(
        f"{t['latest']} {t[salary_type]}", latest_nom_fmt, lang=lang
    )
    kpi_q = create_kpi_card(
        t["q_var"],
        f"{format_localized(metrics['q_nom'], lang=lang, decimals=1)}%",
        nom_variation=metrics["q_nom"],
        ipc_variation=metrics["q_ipc"],
        real_variation=metrics["q_real"],
        lang=lang,
    )
    kpi_a = create_kpi_card(
        t["a_var"],
        f"{format_localized(metrics['a_nom'], lang=lang, decimals=1)}%",
        nom_variation=metrics["a_nom"],
        ipc_variation=metrics["a_ipc"],
        real_variation=metrics["a_real"],
        lang=lang,
    )
    kpi_i = create_kpi_card(
        t["i_var"],
        f"{format_localized(metrics['i_nom'], lang=lang, decimals=1)}%",
        nom_variation=metrics["i_nom"],
        ipc_variation=metrics["i_ipc"],
        real_variation=metrics["i_real"],
        lang=lang,
    )
    ...
    if show_ref and not df_cba_cbt.empty:
        ...
        df_ref_real_full = scraper.calculate_real_salary(
            pd.DataFrame({"ref": df_ref_nom}), ipc_series, base_date=base_date
        )
    ...
    if not is_base100:
        if show_nominal:
            fig_hist.add_trace(
                go.Scatter(
                    x=df_nom_filt.index,
                    y=df_nom_filt[selected_province],
                    name=t["nom_trace"],
                    line=dict(color="#2c3e50", width=3),
                )
            )
            ...
            fig_hist.add_trace(
                go.Scatter(
                    x=df_real_filt.index,
                    y=df_real_filt[selected_province],
                    name="Real",
                    line=dict(color="#18bc9c", width=3),
                )
            )
    ...
    if HAS_ANALYTICS:
        prov_anoms = df_anomalies[
            (df_anomalies["province"] == selected_province)
            ...
            if not prov_anoms.empty:
                ...
                if is_base100:
                    if show_nominal:
                        ...
                        val_nom_base = df_nom.loc[b_date_dt, selected_province]
                        anom_y = (
                            df_nom.loc[prov_anoms["date"], selected_province]
                            / val_nom_base
                        ) * 100
                    else:
                        b_date_dt = pd.to_datetime(base_date)
                        val_real_base = df_real.loc[b_date_dt, selected_province]
                        anom_y = (
                            df_real.loc[prov_anoms["date"], selected_province]
                            / val_real_base
                        ) * 100
                else:
                    if show_nominal:
    ...
    fig_comp = go.Figure(
        go.Bar(
            ...
            marker_color=colors,
            text=[
                f"${format_localized(x, lang=lang)}" if not pd.isna(x) else "-"
                for x in comp_data.values
            ],
    ...
    if HAS_ANALYTICS:
        df_long_c = df_real_filt.T.reset_index().rename(
            columns={df_real_filt.T.reset_index().columns[0]: "province"}
        )
        df_long_c = df_long_c.merge(df_clusters, on="province").melt(
            id_vars=["province", "cluster"], var_name="date", value_name="real_salary"
        )
        ...
        if lang_report["intro"]:
            all_slides.append(
                dbc.Card(
                    [
                        dbc.CardHeader(
                            "Introduction" if lang == "en" else "Introducción",
        ...
        for i, section_text in enumerate(lang_report["clusters"]):
            ...
            if title_match:
                ...
                body_text = re.sub(
                    r"## \*\*Cluster \d:.*?\*\*", "", section_text
                ).strip()
            else:
                card_title = (
                    f"Deep Dive: Cluster {display_idx}"
                    if lang == "en"
                    else f"Análisis: Cluster {display_idx}"
                )
            ...
            if is_base100:
                ...
                base_vals = df_cls[df_cls["date"] == START_LIMIT].set_index("province")[
                    "real_salary"
                ]
                df_cls = df_cls.copy()
                df_cls["index_val"] = df_cls.apply(
                    lambda row: (
                        (row["real_salary"] / base_vals.get(row["province"], 1)) * 100
                    ),
                    axis=1,
                )
                y_val_col = "index_val"
                ...
                df_nom_long = df_nom_filt.reset_index().melt(
                    id_vars="date", var_name="province", value_name="nom_salary"
                )
                df_cls = df_cls.merge(df_nom_long, on=["date", "province"])
                y_val_col = "nom_salary"
                y_axis_title = t["xaxis_salary"]

            fig_cls_small = px.line(
                df_cls,
                x="date",
                y=y_val_col,
                color="province",
                title=f"{card_title} - {t['tab_analytics']}",
                template="plotly_white",
                height=400,
            )
                ...
                html.Div(
                    [
                        dbc.Card(
                            [
                                dbc.CardHeader(
                                    card_title, className="fw-bold bg-light"
                                ),
                                dbc.CardBody(
                                    dcc.Markdown(
                                        body_text,
        ...
        if lang_report["synthesis"]:
            ...
            s_title_match = re.search(
                r"## \*\*Synthesis and Future Outlook: (.*?)\*\*", s_text
            )
            if not s_title_match:
                # Try Spanish header
                s_title_match = re.search(
                    r"## \*\*Síntesis y Perspectivas Futuras: (.*?)\*\*", s_text
                )

            if s_title_match:
                s_card_title = f"{'Synthesis' if lang == 'en' else 'Síntesis'}: {s_title_match.group(1)}"
                s_body_text = re.sub(
                    r"## \*\*(Synthesis and Future Outlook|Síntesis y Perspectivas Futuras):.*?\*\*",
                    "",
                    s_text,
                ).strip()
            else:
                s_card_title = (
                    ...
                    if lang == "en"
                    else "Síntesis y Perspectivas Futuras"
                )
                s_body_text = s_text
            ...
            all_slides.append(
                dbc.Card(
                    [
                        dbc.CardHeader(
                            s_card_title, className="fw-bold bg-dark text-white"
                        ),
                        dbc.CardBody(
                            dcc.Markdown(
                                s_body_text,
...
def run_agent_process(query, history, province, s_type, start, end, lang):
    ...
    try:
        ...
        response = journalist_agent.query(
            query, context_metadata=context, chat_history=history[:-1]
        )
