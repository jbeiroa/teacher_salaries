...
def grade_response(question, agent_answer, ground_truth):
    ...
    try:
        ...
        return result_dict.get("score", 0), result_dict.get("reasoning", "Failed to parse reasoning")
...
def run_evaluation():
    ...
    eval_cases = []
    dataset_path = os.path.join(os.path.dirname(__file__), "..", "tests", "eval_dataset.jsonl")
        ...
        with mlflow.start_run(run_name=f"Model_{m_params['model'].split('/')[-1]}"):
            ...
            for i, case in enumerate(eval_cases):
                ...
                try:
                    ...
                    out = agent.query_and_log(case['q'], ground_truth=case['gt'], chat_history=chat_history)
